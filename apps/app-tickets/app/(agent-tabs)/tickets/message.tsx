...
export default function MessageAgent() {
    ...
    const { loading, error, sendMessageAgent, getMessagesTicketID } = useAgentMessage();
    const { user } = useUser();
    ...
    const loadMessages = async () => {
        if (!id) return;
        const msgs = await getMessagesTicketID(id);
        if (msgs) setMessages(msgs);
    }

    const handleSend = async () => {
        if (!inputValue.trim() || !id || loading) return;
        const result = await sendMessageAgent(id, inputValue);
        if (result) {
            setInputValue("");
            await loadMessages();
        }
        ...
        <View style={styles.container}>
            {messages.length === 0 && !loading ? (
                <View style={styles.emptyContainer}>
                    ...
                    <Text style={styles.emptySubtext}>Envía una solución para este ticket</Text>
                </View>
            ) : (
                <FlatList
                    data={messages}
